...
def llm_semantic_mapping(client, model, canonical_templates, new_templates):
    """Uses LLM to map a list of new templates to existing canonical templates."""
    
    canonical_formatted = []
    for c_id, c in canonical_templates.items():
        canonical_formatted.append(f"ID: {c_id}\nName: {c.get('short_name', '')}\nDescription: {c.get('guideline_description', '')}")
    ...
    for n in new_templates:
        ...
        if not n_id:
            n_id = f"TEMP_{hash(n.get('guideline_description', ''))}"
            n['id'] = n_id
            
        new_formatted.append(f"ID: {n_id}\nName: {n.get('short_name', '')}\nDescription: {n.get('guideline_description', '')}")
        
    system_prompt = """You are an expert mapping agent.
You are given a list of Canonical Guidelines and a list of New Guidelines.
Your task is to map each New Guideline to the corresponding Canonical Guideline ONLY if their semantic descriptions represent the exact same rule or concept, regardless of naming differences.
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate multiple Language Templates Semantically.")
    parser.add_argument('template_files', metavar='F', type=str, nargs='*', help='Path(s) to language_template.json files')
    parser.add_argument('--output', '-o', type=str, default='semantic_template_agreement.json', help='Path to output JSON results.')
    ...
    for fp in template_files:
        ...
        with open(fp, 'r', encoding='utf-8') as f:
            ...
            try:
                data = json.loads(content)
                guidelines = data.get("guidelines", list(data.values())[0] if isinstance(data, dict) else data)
    ...
    if len(templates_by_file) < 2:
        logging.error("Not enough valid files.")
        return
    ...
    for key, appearances in global_guidelines_map.items():
        ...
        match_info = {
            "canonical_id": key,
            "canonical_name": canonical_templates[key].get("short_name", ""),
            "canonical_description": canonical_templates[key].get("guideline_description", ""),
            "files_present_indices": present_in_indices,
            "files_present_paths": list(appearances.keys()),
            "original_descriptions_by_file_index": {
                file_path_to_index[fp]: template.get("guideline_description", "")
        ...
        if len(appearances) > 1:
            ...
            # Basic average similarity metric for the first two items in the cluster
            sim_score = jaccard_similarity(
                t_list[0].get("involved_fragments", ""), 
                t_list[1].get("involved_fragments", "")
            )
            similarity_scores.append(sim_score)
            
            detailed_matches[key] = match_info
            detailed_matches[key]["involved_fragments_similarity_score"] = sim_score

    average_involved_fragments_similarity = sum(similarity_scores) / len(similarity_scores) if similarity_scores else 0.0
    ...
    results = {
        "metrics": {
            ...
            "total_unique_semantic_templates": total_unique_templates,
            "semantic_templates_in_all_files": len(perfect_overlaps),
            "semantic_templates_in_multiple_but_not_all": len(partial_overlaps),
            "semantic_template_agreement_ratio": template_agreement_ratio,
            "average_involved_fragments_jaccard_similarity": average_involved_fragments_similarity
        },
    ...
    logging.info(f"Compared {num_files} language template files using {ai_model}.")
    logging.info(f"Total Unique Templates identified semantically: {total_unique_templates}")
    logging.info(f"Templates found in ALL files: {len(perfect_overlaps)}")
    logging.info(f"Overall Semantic Agreement Ratio: {template_agreement_ratio:.2%}")
    logging.info(f"Average Fragments Jaccard Similarity (for overlaps): {average_involved_fragments_similarity:.2%}")
